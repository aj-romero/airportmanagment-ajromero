...
public class AirPlaneUI {
    public static void printAll(List<AirPlane> airPlanes){
        System.out.printf("%n%-5s | %-20s | %-20s | %-20s | %-10s%n","ID", "Num Registro", "Aerolinea","Capacidad(Pasajeros)","Estado");
    ...
    public static void printMenuCRUD(){
        System.out.printf("%n%n | %-10s | %-10s | %-10s | %-20s | %-10s%n","[1] Agregar", "[2] Editar", "[3] Eliminar", "[4] Regresar al menu principal","[5] Salir");
    ...
    public static void editarAction(){
        ...
        while (!checkValidId(id)){
            System.out.printf("%n%n | %-15s %n","Ingresar el numero correspondiente al avion que desea editar");
    ...
    public static void eliminarAction(){
        int id = -1;
        while (!checkValidId(id)){
            System.out.printf("%n%n | %-15s %n","Ingresar el numero correspondiente al avion que desea eliminar");
            id = UtilsUi.checkInput(DashboardUI.sc);
            if(!checkValidId(id)){
        ...
        System.out.printf("%n | %-15s %s [1]?","Esta seguro de eliminar el avion con registro ",airPlane.getRegisterNumber());
        if(UtilsUi.checkInput(DashboardUI.sc) == 1){
            System.out.printf("%n | %-15s %s exitosamente!","Se elimino el avion con registro  ",airPlane.getRegisterNumber());
    ...
    public static AirPlane selectAirPlane(){
        int id = -1;
        while (!checkValidId(id)){
            System.out.printf("%n%n | %-15s %n","De la siguiente lista favor seleccione el ID del avion a asignar");
            printAll(DashboardUI.aipservice.findAll());
            id = UtilsUi.checkInput(DashboardUI.sc);
            if(isAvaible(id)){
                id = -1;
                System.out.printf("%n | %-15s %n","Verifique el numero ingresado! el avion ya ha sido asignado a una puerta");
            }
            else if(!DashboardUI.aipservice.find(id).getState().getStateName().equals("disponible") ){
                id = -1;
                System.out.printf("%n | %-15s %n","Verifique el numero ingresado! el avion no esta disponible para ser asignado");
            }
        }
    ...
    public static boolean isAvaible(int id){
        ...
        for(int i=0; i < DashboardUI.aipservice.findAll().size(); i++){
            if(a.contains(DashboardUI.aipservice.find(i)) && i == id)
                res = true;
        }
        return res;
    }
